...
def hourly_exp_heat():
    ...
    for row in values[1:]:
        # print(row)
        day = datetime.strptime(row[1][:10], '%d/%m/%Y').strftime('%A')
        dayDict.setdefault(day, []).append(row[1][11:13])
    # print(dayDict)
    #for d in daysOfWeek:
    ...
    layout = go.Layout(
        title='Experiences Transactions Per Hour',
        xaxis = dict(title='Day of Week'),
        yaxis = dict(title='Hour of Day')
    )
    ...
    py.plot(fig, filename='experiences-heatmap')


def trace_graph_spa(value):
    ...
    fig = go.Figure(data=data, layout=layout)
    py.plot(fig, filename='Spa Budget')


def trace_graph_exp(value):
    ...
    fig = go.Figure(data=data, layout=layout)
    py.plot(fig, filename='Experiences Budget')